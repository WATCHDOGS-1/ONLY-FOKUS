  ...
  return (
    <main style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', padding: '32px 16px', position: 'relative' }}>
      <motion.div 
        ...
        style={{ ...glassStyle, width: '100%', maxWidth: '440px', padding: '40px', position: 'relative', zIndex: 10 }}
      >
        <IridescentOverlay />
        <div style={{ position: 'relative', zIndex: 10 }}>
            ...
            {[1, 2, 3].map((s) => (
              <div 
                ...
                style={{
                  ...
                  backgroundColor: s === step ? '#9333EA' : s < step ? 'white' : 'rgba(255,255,255,0.15)'
                }}
              />
            ))}
          </div>
          ...
          <form onSubmit={handleSubmit} style={{ minHeight: '200px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
              ...
              <AnimatePresence mode="wait" custom={1}>
                {step === 1 && (
                  <motion.div key="1" custom={1} variants={variants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.3, ease: [0.25, 0.1, 0.25, 1] }} style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    ...
                    <input required placeholder="aditya_codes" value={formData.username} onChange={(e) => setFormData(p => ({...p, username: e.target.value.toLowerCase().replace(/[^a-z0-9_]/g, '')}))} 
                      style={{ width: '100%', backgroundColor: 'rgba(0,0,0,0.4)', border: '1px solid var(--glass-border)', color: 'white', borderRadius: '12px', padding: '14px 16px', outline: 'none', transition: 'border-color 0.2s', fontSize: '16px' }} 
                ...
                {step === 2 && (
                  <motion.div key="2" custom={1} variants={variants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.3, ease: [0.25, 0.1, 0.25, 1] }} style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    ...
                    <textarea required maxLength={160} placeholder="Building the next big thing..." value={formData.bio} onChange={(e) => setFormData(p => ({...p, bio: e.target.value}))} rows={3} 
                      style={{ width: '100%', backgroundColor: 'rgba(0,0,0,0.4)', border: '1px solid var(--glass-border)', color: 'white', borderRadius: '12px', padding: '14px 16px', outline: 'none', transition: 'border-color 0.2s', resize: 'none', fontSize: '16px' }}
                      onFocus={e => e.currentTarget.style.borderColor = '#9333EA'}
                      onBlur={e => e.currentTarget.style.borderColor = 'var(--glass-border)'}
                    />
                ...
                {step === 3 && (
                  <motion.div key="3" custom={1} variants={variants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.3, ease: [0.25, 0.1, 0.25, 1] }} style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', gap: '12px', alignItems: 'center', textAlign: 'center' }}>
                    ...
                    <div style={{ width: '96px', height: '96px', borderRadius: '50%', backgroundColor: 'rgba(0,0,0,0.4)', border: '2px solid #9333EA', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '30px', fontWeight: 'bold', color: '#9333EA', boxShadow: '0 0 30px rgba(147,51,234,0.4)', marginBottom: '16px' }}>
            ...
            <motion.button 
              ...
              style={{ width: '100%', marginTop: '40px', padding: '14px 24px', borderRadius: '12px', background: 'linear-gradient(135deg, #7C3AED, #9333EA)', boxShadow: '0 0 30px rgba(147,51,234,0.4)', color: 'white', fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', outline: 'none', border: 'none', cursor: (loading || (step === 1 && !formData.username) || (step === 2 && !formData.bio)) ? 'not-allowed' : 'pointer', opacity: (loading || (step === 1 && !formData.username) || (step === 2 && !formData.bio)) ? 0.5 : 1, transition: 'background-color 0.2s, opacity 0.2s' }}
            >
              {loading ? <div style={{ width: '20px', height: '20px', borderRadius: '50%', border: '2px solid rgba(255,255,255,0.3)', borderTopColor: 'white' }}/> : step === totalSteps ? 'Complete Setup' : 'Continue'}
            </motion.button>
          </form>
        </div>
      </motion.div>
